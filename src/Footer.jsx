...
// Custom text-based logo component (matches Navbar)
const Logo = ({ isDark }) => (
  <div className="flex items-center gap-2">
    ...
    <div className="relative w-7 h-7 flex items-center justify-center">
      {/* Outer ring */}
      <div
        className={`absolute inset-0 rounded-md border-2 rotate-45 transition-colors ${
...
export default function Footer() {
  ...
  return (
    <footer
      className={`w-full border-t py-16 relative z-10 ${
        isDark ? 'border-slate-800/50 bg-dark-900' : 'border-slate-200 bg-white'
      }`}
    >
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-12">
          ...
          <div>
            ...
            <p className={`text-sm max-w-xs ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
              Transforming regulatory compliance from months to minutes with AI-powered automation.
            </p>
          </div>
        ...
        >
          <div className={`text-sm ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
            © 2026 RegRadar. All rights reserved.
          </div>

          <div className="flex items-center gap-6">
            <span className={`text-sm ${isDark ? 'text-slate-600' : 'text-slate-500'}`}>
              Built by <span className="text-brand-500 font-semibold">Team CoreDump</span>
            </span>
            ...
            <div className="flex items-center gap-3">
              <a
                href="#"
                className={`p-2 transition-colors ${isDark ? 'text-slate-600 hover:text-slate-300' : 'text-slate-400 hover:text-slate-600'}`}
              >
